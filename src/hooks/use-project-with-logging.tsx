...
export function useProjectWithLogging() {
  ...
  useEffect(() => {
    ...
    console.log('Loaded saved project ID from localStorage:', savedProjectId);
    ...
    if (savedProjectId && projects && projects.length > 0) {
      ...
      if (savedProject) {
        ...
        console.log('Saved project not found in available projects');
        if (projects.length > 0) {
          console.log('Setting first available project instead:', projects[0]);
          setCurrentProject(projects[0]);
          localStorage.setItem('selectedProjectId', projects[0].id);
        }
      }
