...
const ExperimentList = ({
  ...
  if (experiments.length === 0) {
    return <EmptyExperimentState onCreateNew={onCreateNew} isGrowthExperiment={isGrowthExperiment} />;
  }
  ...
  return <div className="space-y-4">
      <Table>
        ...
        <TableBody>
          {experiments.map(experiment => <TableRow key={experiment.id} className="cursor-pointer hover:bg-gray-50" onClick={() => onViewDetail(experiment)}>
              ...
              <TableCell>
                {experiment.results ? <p className="truncate max-w-xs">{experiment.results}</p> : <span className="text-gray-400">No results yet</span>}
              </TableCell>
